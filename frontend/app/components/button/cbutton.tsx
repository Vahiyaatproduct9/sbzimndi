...
const Cbutton = ({ containerStyle, style, pressedStyle, textStyle, pressed, text, pressedState, onPress }: prop) => {
    return (

        <View style={[css.containerStyle, containerStyle]}>
            <Pressable onPressIn={pressedState && (() => pressedState(true))}
                onPressOut={pressedState && (() => pressedState(false))}
                onPress={onPress ? onPress : () => { }}
                style={[css.saveBtn, style, pressed ? [css.buttonPressed, pressedStyle] : null]}>
                <Text style={[css.textStyle, textStyle]}>{text}</Text>
            </Pressable>
        </View>
    )
}
