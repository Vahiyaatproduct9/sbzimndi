    ...
    useEffect(() => {
        const getaccess_token = async () => {
            ...
            if (access_token && access_token.length > 0 && refresh_token && refresh_token.length > 0) {
                setLogged(true)
                console.log('access_token from profile.tsx', access_token)
            }
            else {
                setLogged(false)
            }
            ...
            <Stack.Navigator>
                {logged === false ? (
                    <>
                        ...
                        <Stack.Screen name='otp' initialParams={{ setActiveTab, setLogged }} component={Otp} options={{ headerShown: false }} />
