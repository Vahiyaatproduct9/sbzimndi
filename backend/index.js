...
app.post('/addpost', upload.single('photo'), async (req, res) => {
    ...
    console.log('log from index -->', info, photo)
...
//     const reS = req.body;
//     const { data, error } = await checkUser(reS.access_token)
//     console.log('checkUser -->', reS, data, error)
//     if (!data.user && (error.status === 400 || error.status === 401)) {
//         res.sendStatus(400)
//     }
//     else if (data.user) {
//         res.sendStatus(200)
//     }
//     else return res.sendStatus(error.status)
// })
